...
def data_persist_sql(spark,df,dfName,url,driver,dbtable,mode,user,password):
    try:
        logger.info("Data Persist Script - data_persist() is started for saving dataframe "+ dfName + " into PostgresSQL Table...")
        df.write.format("jdbc").option("url",url).option("driver",driver)\
                .option("dbtable",dbtable).mode(mode).option("user",user)\
                .option("password",password).save()
    except Exception as exp:
        logger.error("Error in the method - data_persist_sql(). Please check the Stack Trace. " + str(exp),exc_info=True)
        raise
    else:
        logger.info("Data Persist - data_persist() is completed for saving dataframe "+ dfName +" into PostGresql Table...")
